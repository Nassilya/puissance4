...
# ----------- Paramètres du tournoi pour IA moyenne -----------
comparaisons_ia = [
    ("moyen", "facile"),
    ("moyen", "moyen"),
    ("moyen", "difficile")
...
plt.barh(noms, valeurs, color='#dda0dd')  # violet clair
plt.xlabel("Temps moyen par match (secondes)")
plt.title("emps d’exécution moyen par confrontation IA (Moyenne, affiché pour 50 matchs).")
plt.tight_layout()
plt.show()
...
for i, (titre, ia1, vic1, ia2, vic2) in enumerate(statistiques):
    axs[i].pie(
        ...
        colors=["#800080", "#dda0dd"],  # violet et violet clair
        autopct='%1.1f%%',
        startangle=90
    )
...
print("\n--- Résumé du tournoi IA Moyenne ---")
print("Confrontation      | Victoires IA1 | Victoires IA2 | Temps moyen (s)")
for (titre, ia1, vic1, ia2, vic2), (_, tps) in zip(statistiques, temps_execution):
    print(f"{titre:18} | {vic1:13} | {vic2:13} | {tps:14.2f}")